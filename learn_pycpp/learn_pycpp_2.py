...
g++ -shared -fPIC -std=c++11 -O3 $HOME/anaconda3/lib/libpython3.6m.a cpp_nms.cpp  -o cpp_nms.so -I$HOME/anaconda3/include/python3.6m -lboost_python36 -lboost_numpy36
'''

'''
# third use .os in python
from cpp_nms import cpp_nms
'''
import numpy as np
...
# jit_nms = jit(np_nms)
# @jit
# def jit_nms(boxes, scores, thresh = 0.45):
#     x1, y1, x2, y2 = boxes[:,0], boxes[:,1], boxes[:,2], boxes[:,3]
#     # areas = (x2 - x1 + 1) * (y2 - y1 + 1)
#     areas = (x2 - x1) * (y2 - y1)
#     order = scores.argsort()[::-1]
...
if __name__=='__main__':
    ...
    #     boxes_cpu_ph = tf.placeholder(dtype=tf.float32, shape=[nboxes, 4])
    #     scores_cpu_ph = tf.placeholder(dtype=tf.float32, shape=[nboxes])
    #     cpu_result = tf.image.non_max_suppression(boxes_cpu_ph, scores_cpu_ph, max_output_size=100000, iou_threshold=thresh)
    # tfgpu
    # boxes_gpu_ph = tf.placeholder(dtype=tf.float32, shape=[nboxes, 4])
    #scores_gpu_ph = tf.placeholder(dtype=tf.float32, shape=[nboxes])
    #gpu_result = tf.image.non_max_suppression(boxes_gpu_ph, scores_gpu_ph, max_output_size=100000, iou_threshold=thresh) 
