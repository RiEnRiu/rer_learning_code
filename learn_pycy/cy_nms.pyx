...
def cy_nms(np.ndarray[np.float32_t, ndim=2] boxes, np.ndarray[np.float32_t, ndim=1] scores, np.float thresh):
    ...
    cdef int nboxes = boxes.shape[0]
    cdef np.ndarray[np.int_t, ndim=1] suppressed = \
            np.zeros((nboxes), dtype=np.int)
